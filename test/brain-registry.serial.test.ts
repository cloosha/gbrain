...
describe('BrainRegistry — lazy init', () => {
  ...
  test('empty/null/undefined id routes to host', async () => {
    // Isolate the host config path: developer machines may have a real ~/.gbrain/config.json,
    // but this test only needs to prove routing reaches host init rather than UnknownBrainError.
    const tmp = mkdtempSync(join(tmpdir(), 'brain-registry-host-'));
