  ...
  while (!stopping) {
    ...
    try {
      ...
      try {
        ...
        // LOCAL PATCH 2026-04-22: upstream passed no args; connect() reads
        // config.poolSize so null config threw "undefined is not an object".
        // Pass cfg from outer scope. Upstream bug reproducible in v0.12.0.
        await (engine as any).connect?.(cfg);
...
function writeWrapperScript(repoPath: string, args: string[] = []): string {
  const home = process.env.HOME || '';
  ...
  const interval = parseArg(args, '--interval') || '3600';
  if (!/^\d+$/.test(interval)) {
    throw new Error(`Invalid --interval "${interval}". Expected seconds as an integer.`);
  }
...
cd '${safeRepoPath}' || exit 1

if [ -z "\${OPENAI_API_KEY:-}" ] && command -v jq >/dev/null 2>&1; then
  OPENAI_API_KEY="$(jq -r '.openai_api_key // empty' "$HOME/.gbrain/config.json" 2>/dev/null)"
  export OPENAI_API_KEY
fi

export GBRAIN_EMBED_CONCURRENCY="\${GBRAIN_EMBED_CONCURRENCY:-4}"
export GBRAIN_EMBED_TIMEOUT_MS="\${GBRAIN_EMBED_TIMEOUT_MS:-15000}"
export GBRAIN_EMBED_MAX_RETRIES="\${GBRAIN_EMBED_MAX_RETRIES:-2}"
export GBRAIN_DB_POOL_SIZE="\${GBRAIN_DB_POOL_SIZE:-2}"
export GBRAIN_DB_CONNECT_TIMEOUT="\${GBRAIN_DB_CONNECT_TIMEOUT:-30}"
export GBRAIN_MINION_LOCK_DURATION_MS="\${GBRAIN_MINION_LOCK_DURATION_MS:-300000}"
export GBRAIN_MINION_STALLED_INTERVAL_MS="\${GBRAIN_MINION_STALLED_INTERVAL_MS:-60000}"

exec '${safeGbrainPath}' autopilot --repo '${safeRepoPath}' --interval '${interval}'
`;
...
async function installDaemon(engine: BrainEngine, args: string[]) {
  ...
  const wrapperPath = writeWrapperScript(repoPath, args);
  const home = process.env.HOME || '';
...
function showStatus(json: boolean) {
  ...
  if (process.platform === 'darwin') {
    if (existsSync(plistPath())) targets.push('macos');
  } else {
    const systemdPath = join(process.env.HOME || '', '.config', 'systemd', 'user', 'gbrain-autopilot.service');
    if (existsSync(systemdPath)) targets.push('linux-systemd');
    try {
  ...
  if (json) {
    console.log(JSON.stringify({ installed, targets, last_log: lastLine }));
  } else {
    console.log(`Autopilot: ${installed ? `installed (${targets.join(', ')})` : 'not installed'}`);
